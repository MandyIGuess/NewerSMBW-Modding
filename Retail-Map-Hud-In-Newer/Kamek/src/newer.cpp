...
static const wchar_t *numberKinds[] = {
	// 0-19 are handled by code
	// To insert a character: \x0B\x0FYY\xZZZZ
	// F is the font ID, YY is the character code, ZZZZ is ignored
	L"A", // 20, alternate (uses marioFont)
	L"\x0B\x0148\xBEEF", // 21, tower
	L"\x0B\x0148\xBEEF", // 22, tower (formerly tower 2)
	L"\x0B\x012E\xBEEF", // 23, castle
	L"\x0B\x012F\xBEEF", // 24, fortress
	L"\x0B\x013D\xBEEF", // 25, final castle
	L"\x0B\x014D\xBEEF", // 26, train
	...
	L"Palace", // 28, switch palace
	L"\x0B\x0147\xBEEF", // 29, yoshi's house
	L"\x0B\x014E\xBEEF", // 30, key
	L"\x0B\x014E\xBEEF", // 31, key
	L"\x0B\x014E\xBEEF", // 32, key
	L"\x0B\x014E\xBEEF", // 33, key
	L"\x0B\x014E\xBEEF", // 34, key
	L"\x0B\x014E\xBEEF", // 35, key
	L"\x0B\x0138\xBEEF", // 36, music house
	L"\x0B\x0133\xBEEF", // 37, shop
	L"\x0B\x0139\xBEEF", // 38, challenge house
	L"\x0B\x0151\xBEEF", // 39, red switch palace
	...
	L"\x0B\x0153\xBEEF", // 41, yellow switch palace
	L"\x0B\x0154\xBEEF", // 42, green switch palace
	L"\x0B\x0136\xBEEF", // 43, dot (stop node)
	L"\x0B\x0134\xBEEF", // 44, start arrow right
};
...
void getWorldNameAndLevelNumString(int level, wchar_t *dest) {
	dest[0] = '-';
	if (level >= 20) {
		wcscpy(&dest[1], numberKinds[level-20]);
	} else if (level >= 10) {
		dest[1] = '1';
		dest[2] = ('0' - 10) + level;
		dest[3] = 0;
